...
def read_config(cfile):
  ...
  with open(cfile,"r") as file_object:
    config_object.read_file(file_object)
    CONFIG['defaults'] = {'skip_noduedate': (config_object['daily_caldav']).getboolean('skip_noduedate', True),
                           'get_overdue': (config_object['daily_caldav']).getboolean('get_overdue', True),
                           'get_futuredue': (config_object['daily_caldav']).getboolean('get_futuredue', True),
                           'get_withoutdue': (config_object['daily_caldav']).getboolean('get_withoutdue', False),
                           'get_all': (config_object['daily_caldav']).getboolean('get_all', False),
                           'futuredue_afterdate': (config_object['daily_caldav']).get('futuredue_afterdate', '+3'),
                           'futuredue_aftermsg': (config_object['daily_caldav']).get('futuredue_aftermsg', '(%v %b)'),
                           'withoutdue_afterdate': (config_object['daily_caldav']).get('withoutdue_afterdate', ''),
                           'withoutdue_aftermsg': (config_object['daily_caldav']).get('withoutdue_aftermsg', ''),
                           'show_priority': (config_object['daily_caldav']).getboolean('show_priority', True),
                           'show_priority_char': (config_object['daily_caldav']).get('show_priority_char', '!'),
                           'show_overdue': (config_object['daily_caldav']).getboolean('show_overdue', True),
                           'show_overdue_msg': (config_object['daily_caldav']).get('show_overdue_msg', '[overdue since DATE]'),
                          }
    CONFIG['resource'] = []
    for res in config_object['daily_caldav']['resources'].split():
      resconf = f'daily_caldav_{res}'
